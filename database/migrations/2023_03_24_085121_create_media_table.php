...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('media', function (Blueprint $table) {
            ...
            $table->string('subdirectory')->nullable()->comment('メディア格納先サブディレクトリ');
            $table->string('filename')->comment('メディアコンテンツファイル名');
            $table->string('uri')->nullable()->unique()->comment('メディアURI');
            $table->integer('rounds')->unsigned()->default(0)->comment('メディアURI生成用エンコードラウンド');
